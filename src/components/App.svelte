<script>
  ...
  onMount(async () => {
    ...
    selectedCountries = [...selectedCountries, 'Iceland']
  });
...
<main on:click={closeDropdown}>
  <h1 style="line-height: 1.25; text-align: left;">Pioneering Sustainability: Iceland's Per Capita Energy Consumption Surpasses the World's Average,<br>Powered by Low Carbon Innovations</h1>
  <h2>Primary Energy consumption per capita and percentages of different sources since 1965 collected by <a class="custom-link" href="https://github.com/owid/energy-data?tab=readme-ov-file#data-on-energy-by-our-world-in-data">Our World in Data</a></h2>
  <div style="margin-top: 10px; text-align: left;"  class="dropdown" on:click={toggleDropdown}>
    <input type="text" placeholder="Search a country" on:input={handleInput} bind:value={inputText} on:focus={toggleDropdown} on:input={toggleDropdown}>
